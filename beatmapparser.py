...
class BeatmapParser():
    ...
    def parse_timing_point(self, line):
        ...
        if not math.isnan(timing_point["beatLength"]) and timing_point["beatLength"] != 0:
            if timing_point["beatLength"] > 0:
                ...
                self.beatmap["bpmMin"] = min(
                    self.beatmap["bpmMin"], bpm) if "bpmMin" in self.beatmap else bpm
                self.beatmap["bpmMax"] = max(
                    self.beatmap["bpmMax"], bpm) if "bpmMax" in self.beatmap else bpm
                timing_point["bpm"] = bpm
            else:
                # If negative, beatLength is a velocity factor
                timing_point["velocity"] = abs(
                    100 / timing_point["beatLength"])
    ...
    def parse_hit_object(self, line):
        ...
        if object_type & 1:
            ...
            if timing:
                px_per_beat = float(
                    self.beatmap["SliderMultiplier"]) * 100 * float(timing["velocity"])
                beats_number = (
                    hit_object["pixelLength"] * int(hit_object["repeatCount"])) / px_per_beat
                hit_object["duration"] = math.ceil(
                    beats_number * timing["beatLength"])
                hit_object["end_time"] = hit_object["startTime"] + \
                    hit_object["duration"]

            # Parse slider points
            points = (members[5] or '').split('|')
            if len(points):
                hit_object["curveType"] = self.curve_types[points[0]
                                                           ] or 'unknown'
            ...
            if end_point and end_point[0] and end_point[1]:
                if isinstance(end_point[0], str) or isinstance(end_point[1], str):
                ...
                hit_object["end_position"] = hit_object["points"][len(
                    hit_object["points"]) - 1]
    ...
    def parse_event(self, line):
        ...
        if members[0] == '0' and members[1] == '0' and members[2]:
            ...
            if bg_name[0] == '"' and bg_name[len(bg_name) - 1] == '"':
                self.beatmap["bg_filename"] = bg_name.substring(
                    1, bg_name.length - 1)
            else:
    ...
    def compute_duration(self):
        ...
        last_object = self.beatmap["hitObjects"][len(
            self.beatmap["hitObjects"]) - 1]
        ...
        if first_object and last_object:
            self.beatmap["total_time"] = math.floor(
                last_object["startTime"] / 1000)
    ...
    def compute_max_combo(self):
        ...
        next_offset = timing_points[1]["offset"] if len(
            timing_points) > 1 else math.inf
        i = 1
        ...
        for hit_object in self.beatmap["hitObjects"]:
            ...
            osupx_per_beat = slider_multiplier * \
                100 * current_timing["velocity"]
            ...
            if hit_object["object_name"] == 'spinner' or hit_object["object_name"] == 'circle':
                ...
                tick_per_side = math.ceil(
                    (math.floor(hit_object["pixelLength"] / tick_length * 100) / 100) - 1)
    ...
    def build_beatmap(self):
        ...
        self.beatmap["breakTimes"].sort(
            key=lambda a, b: 1 if a.startTime > b.startTime else -1)
        ...
        for i in range(1, len(timing_points)):
            if not "bpm" in timing_points[i]:
                timing_points[i]["beatLength"] = timing_points[i -
                                                               1]["beatLength"]
